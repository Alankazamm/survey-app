...
*/
enum AccountStatus {
    NOT_CREATED = "I have not created an account yet.",
    OKAY = "My account status is okay.",
    FINANCIAL_ISSUES = "I'm currently having financial issues."
}
enum Market {
    STOCK = "stock market",
...
enum Invest {
    NOT_INVESTED = "I have not invested yet.",
    BANK_MANAGER = "My bank's manager advices me.",
    FLIP_COIN = "I flip a coin"
}
interface Invests{
    [key: string]: {
      [key: string]: number;
...
const surveys: Infos[] = await getAllSurveys();
...
Object.keys(investmentByDate).forEach(market => {
    let accumulatedInvestment: { [date: string]: number } = {};
    surveys.forEach(survey => {
